...
export type DeleteAzureDevOpsPATModalProps = {
  tokenID: string;
  repositoryIDs: string[];
};

const DeleteAzureDevOpsPATModal = ({ tokenID, repositoryIDs }: DeleteAzureDevOpsPATModalProps) => {
  const [open, setOpen] = useState(false);

  const [deleteAzureDevOpsPAT, { isLoading: isDeletingPAT }] = useDeletePATMutation();
  ...
  const onDelete = async () => {
    try {
      ...
      const response = await deleteAzureDevOpsPAT({ personalAccessTokenID: tokenID });
      ...
      <DialogContent className="w-[420px] max-w-none px-12 py-8 bg-white rounded-xl flex flex-col justify-center items-center gap-6">
        <div className="w-full flex flex-col items-center gap-4">
          ...
          <div className="w-full text-center text-sm leading-[16.8px] text-text-950">
            Are you sure you want to delete the Personal Access Token and disconnect Azure DevOps? Once deleted, the
            token cannot be restored.
          </div>
        </div>
...
export default DeleteAzureDevOpsPATModal;
