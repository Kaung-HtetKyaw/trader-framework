...
import EditAzureDevOpsPATForm, { EditAzureDevOpsPATFormData } from './form';
...
import { useUpdateAzureDevOpsPATMutation } from '@/store/api/gitOpsApi';

type EditAzureDevOpsPATModalProps = {
  tokenID: string;
  owner?: string;
};

const EditAzureDevOpsPATModal = ({ tokenID, owner }: EditAzureDevOpsPATModalProps) => {
  const [open, setOpen] = useState(false);

  const [updateAccessCredential, { isLoading }] = useUpdateAzureDevOpsPATMutation();

  const onSubmit = async (data: EditAzureDevOpsPATFormData, onError?: (error: string) => void) => {
      ...
      >
        <div className="flex flex-col justify-start items-center gap-2 w-full">
          ...
          <p className="text-center text-text-500 body-1 font-normal leading-[1.2] mt-4">
            Update your existing Personal Access Token to maintain your Azure DevOps integration.
          </p>
        </div>
        <div className="flex flex-col justify-start items-start gap-4 w-full">
          <EditAzureDevOpsPATForm
            defaultValues={{ token: '', owner: owner || '' }}
            isLoading={isLoading}
...
export default EditAzureDevOpsPATModal;
