...
export type DeleteGithubRepoModalProps = {
  repositoryID: string;
};

const DeleteGithubRepoModal = ({ repositoryID }: DeleteGithubRepoModalProps) => {
  const [open, setOpen] = useState(false);

  const [deleteGithubRepo, { isLoading: isDeleting }] = useDeleteRepoMutation();
  ...
  const onDelete = async () => {
    ...
    const response = await deleteGithubRepo({ repositoryID });
...
export default DeleteGithubRepoModal;
