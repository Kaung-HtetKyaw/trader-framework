...
import EditGithubRepoForm, { EditGithubRepoFormData } from './form';
import { useUpdateRepoMutation } from '@/store/api/gitOpsApi';

type EditGithubRepoModalProps = {
  repositoryID: string;
  defaultValues: { name: string; description: string };
};

const EditGithubRepoModal = ({ repositoryID, defaultValues }: EditGithubRepoModalProps) => {
  const [open, setOpen] = useState(false);

  const [updateGithubRepo, { isLoading }] = useUpdateRepoMutation();

  const onSubmit = async (data: EditGithubRepoFormData, onError?: (error: string) => void) => {
    const response = await updateGithubRepo({
...
export default EditGithubRepoModal;
